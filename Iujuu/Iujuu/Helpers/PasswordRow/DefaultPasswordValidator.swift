...
open class DefaultPasswordValidator: PasswordValidator {
    ...
    let rules: [PasswordRule] = [
        PasswordRule(hint: "Al menos una minúscula") { $0.satisfiesRegexp("[a-z]") },
        PasswordRule(hint: "Al menos un numéro") { $0.satisfiesRegexp("[0-9]") },
        PasswordRule(hint: "Al menos una mayúscula") { $0.satisfiesRegexp("[A-Z]") },
        PasswordRule(hint: "Al menos 6 caracteres") { $0.characters.count > 5 }
    ]
